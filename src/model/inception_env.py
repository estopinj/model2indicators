...
class InceptionEnv(nn.Module):
    ...
    def __init__(self, n_labels=3336, n_input=80, dropout=0.5, last_layer=True, logit=False, exp=False,
                 normalize_weight=1., temperature=1., normalise_last_layer=False):
        ...
        self.normalise_last_layer = normalise_last_layer

        if normalise_last_layer:
            print("****** Normalisation on last layer")
            self.fc_norm = NormedLinear(2048, n_labels)
    ...
    def forward(self, x):
        ...
        if self.last_layer:
            # Normalisation for LDAM loss
            if self.normalise_last_layer:
                x = self.fc_norm(x)
            else:
                # 2048
                x = self.fc(x)
